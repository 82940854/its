...
;layui.define(["laytpl", "laypage", "layer", "form", "util"], function (e) {
    ...
    F.prototype.config = {
        ...
        var i = this;
        //当前页的数据条数
        var currPageNum =  i.page < i.config.page.pages?i.config.page.limit:(i.config.page.count - (i.page-1)*i.config.page.limit)
        // console.log(i,'000',$('tr .layui-form-checkbox').length,'111',$('tr .layui-form-checked').length,'222',currPageNum)
        if(!(e&&e.page&&e.page.curr)) {
            if(globalDeleteSymbol) {
                if(currPageNum == 1) {
                    i.config.page.curr = i.config.page.curr == 1?1:i.config.page.curr - 1
                }else if(currPageNum > 1) {
                    if($('tr .layui-form-checkbox').length > 0) {
                        if($('tr .layui-form-checkbox').length == $('tr .layui-form-checked').length) {
                            i.config.page.curr = i.config.page.curr == 1?1:i.config.page.curr - 1
                        }
                    }
                }
                ...
                if(i.config.page) {
                    i.config.page.curr = i.config.page.curr
                }
            }
        }
        // var testNum = (i.page-1)*i.config.page.limit;
        // if(testNum+1==i.config.page.count&&i.page>1){if(e&&e.page&&e.page.curr){
        //     e.page.curr=i.page-1;
        // }else if(e&&e.page&&e.page.pageIndex){
        //     e.page.pageIndex=i.page-1;
        // }else {
        //     i.config.page.curr=  i.config.page.curr-1;
        // }
        // }
        globalDeleteSymbol = false;
